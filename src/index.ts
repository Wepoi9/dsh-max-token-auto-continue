...
if (typeof entry !== 'string' || entry.length === 0) {
  throw new Error(
    'dsh-max-token-auto-continue: cannot resolve the DSH runtime entry (process.argv[1])',
  )
}
...
export function apply(ctx: Context, rawConfig: PluginConfig = {}) {
  ...
  const liveRootAgent = (session: Session): Agent | undefined => {
    const agent = ctx.agents.get(session.id)
    if (agent === undefined) return undefined
    if (agent.session !== session) return undefined
    if (!ctx.agents.roots().includes(agent)) return undefined
    return agent
  }

  const continueAfterIdle = async (
    session: Session,
    agent: Agent,
    state: SessionState,
    ticket: number,
  ...
  ctx.on('session/event', (session: Session, event: SessionEvent) => {
    switch (event.type) {
      ...
      case 'turn/end': {
        ...
        if (!enabled) return
        if (state.pendingTurn !== undefined) return
        if (state.consecutive >= maxConsecutive) return
